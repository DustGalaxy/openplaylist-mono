...
export function AccountTab({ user, onUserUpdate }: AccountTabProps) {
  ...
  const handleProfileSubmit = async (e: FormEvent) => {
    ...
    if (!username) {
      toast.error(
        t(
          'settings.account.validation.nicknameRequired',
    ...
    if (username.length < 3) {
      toast.error(
        t(
          'settings.account.validation.nicknameShort',
    ...
    if (!email) {
      toast.error(
        t('settings.account.validation.emailRequired', 'Email is required'),
      )
      return
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      toast.error(
        t('settings.account.validation.emailInvalid', 'Invalid email address'),
      )
      return
    }
    ...
    try {
      ...
      toast.success(
        t('settings.account.profileUpdated', 'Profile updated successfully'),
      )
    } catch (error) {
      toast.dismiss(loadingToast)
      toast.error(
        getApiErrorMessage(
          error,
          t('settings.account.updateFailed', 'Failed to update profile'),
        ),
      )
    } finally {
  ...
  const handlePasswordSubmit = async (e: FormEvent) => {
    ...
    if (!currentPassword || !newPassword || !confirmPassword) {
      toast.error(
        t(
          'settings.account.validation.passwordFillAll',
    ...
    if (newPassword.length < 8) {
      toast.error(
        t(
          'settings.account.validation.passwordShort',
    ...
    if (newPassword !== confirmPassword) {
      toast.error(
        t(
          'settings.account.validation.passwordMismatch',
    ...
    if (currentPassword === newPassword) {
      toast.error(
        t(
          'settings.account.validation.passwordSame',
    ...
    try {
      ...
      toast.dismiss(loadingToast)
      toast.success(
        t('settings.account.passwordChanged', 'Password changed successfully'),
      )
      setFormState((prev) => ({
        ...prev,
        currentPassword: '',
    ...
    <div className="space-y-4">
        ...
        <div>
          ...
          <DialogDescription className="text-xs text-text-secondary mt-0.5">
            {t(
              ...
              'Manage your credentials, email, nickname, and security settings.',
            )}
          </DialogDescription>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <form
          ...
          className="p-3 sm:p-4 border border-accent/60 rounded-md bg-level-1 space-y-3.5 shadow-xs flex flex-col justify-between"
        >
          <div className="space-y-3.5">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-text-placeholder">
                {t(
                  'settings.account.nicknameHelp',
                  'Shown on playlists and public activity.',
                )}
              </p>
            </div>
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                className="bg-text-main text-text-main hover:bg-level-2 hover:text-text-main focus:bg-level-2 focus:text-text-main transition-colors duration-700 border-0 h-8 px-2.5 text-xs sm:text-sm focus-visible:ring-1 focus-visible:ring-accent/50 select-none focus:select-text cursor-pointer focus:cursor-text"
                title="Hover or click to reveal email"
              />
              <p className="text-[11px] text-text-placeholder">
                {t(
                  'settings.account.emailHelp',
            ...
            <div className="space-y-1">
              ...
              <div className="flex gap-2 items-center">
                <Input
                  ...
                  disabled={profileSaving}
                  onChange={(e) =>
                    handleValueChange('profile_image_url', e.target.value)
                  }
                  className="bg-level-2 border-0 h-8 px-2.5 text-xs sm:text-sm focus-visible:ring-1 focus-visible:ring-accent/50 flex-1 min-w-0"
                />
                <div className="size-8 rounded-full bg-level-2 shrink-0 overflow-hidden flex items-center justify-center border border-accent/40">
