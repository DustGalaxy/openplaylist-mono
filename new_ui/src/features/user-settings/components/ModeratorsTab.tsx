...
export function ModeratorsTab() {
  ...
  const [tokenName, setTokenName] = useState('Модератор стрима')
  ...
  const handleCreateToken = async () => {
    try {
      ...
      const created = await createChannelModeratorToken({
        name: tokenName.trim() || 'Модератор стрима',
      ...
      toast.success('Токен модератора успешно создан')
      setModerators((prev) => [fullCreated, ...prev])
      setIsTokenModalOpen(false)
      resetModalForm()
    } catch (err: any) {
      console.error(err)
      toast.error(err.response?.data?.detail || 'Ошибка создания токена')
    } finally {
  ...
  const handleDirectAdd = async () => {
    ...
    try {
      ...
      console.error(err)
      toast.error(err.response?.data?.detail || 'Ошибка добавления модератора')
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const handleClaimToken = async () => {
    if (!claimTokenInput.trim()) {
      toast.error('Введите токен модератора')
      return
    }
    try {
      setIsSubmitting(true)
      ...
      const res = await claimChannelModeratorToken(cleanToken)
      toast.success(`Токен успешно активирован! Вы подключены как модератор.`)
      setIsClaimModalOpen(false)
      setClaimTokenInput('')
      await loadData()
    } catch (err: any) {
      console.error(err)
      toast.error(err.response?.data?.detail || 'Не удалось активировать токен')
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const handleTogglePlayerControl = async (mod: ChannelModeratorResponse, val: boolean) => {
    try {
      ...
      toast.success('Права на плеер обновлены')
    } catch {
  ...
  const handleToggleAllPlaylists = async (mod: ChannelModeratorResponse, val: boolean) => {
    try {
      ...
      toast.success('Права на плейлисты обновлены')
    } catch {
      toast.error('Ошибка обновления прав')
    }
  }
  ...
  const handleGrantPlaylist = async () => {
    ...
    try {
      ...
      toast.success('Доступ к плейлисту предоставлен')
      setModerators((prev) =>
        prev.map((m) => {
      ...
      toast.error(err.response?.data?.detail || 'Ошибка предоставления доступа')
    }
  }
  ...
  const handleRevokePlaylist = async (modId: string, plstId: string) => {
    try {
      ...
      toast.success('Доступ к плейлисту отозван')
      setModerators((prev) =>
        prev.map((m) => {
      ...
      toast.error('Ошибка отзыва доступа к плейлисту')
    }
  }
  ...
  const copyToClipboard = (text: string, isLink = false) => {
    ...
    setCopiedToken(text)
    toast.success(isLink ? 'Ссылка-приглашение скопирована' : 'Токен скопирован')
              ...
              >
                <div className="min-w-0">
                  ...
                  <span className="text-[11px] text-text-secondary">
                    {c.can_control_player && '🎛️ Плеер '}
                    {c.can_manage_all_playlists
                      ? '• Все плейлисты'
                      : `• Плейлистов: ${c.playlist_access?.length || 0}`}
                  </span>
                </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-accent/10 pb-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {mod.user_id ? 'Привязан' : 'Ожидает активации'}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      ...
                      <button
                        ...
                        title="Скопировать готовую ссылку-приглашение"
                      >
                  ...
                  <div className="flex items-center gap-2 self-end sm:self-auto">
                    ...
                    >
                      {selectedMod?.id === mod.id ? 'Скрыть плейлисты' : 'Плейлисты'}
                    </Btn>
                    <button
                      type="button"
                        ...
                        {mod.playlist_access.map((acc) => (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-3">
                              <span className="text-[11px] text-text-secondary">
                                {acc.can_manage_tracks && '🎵 Треки '}
                                {acc.can_manage_settings && '⚙️ Настройки'}
                              </span>
                              <button
                                type="button"
                                onClick={() =>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-level-1 border border-accent/40 rounded-xl p-5 w-full max-w-md space-y-4 shadow-2xl">
            ...
            <div className="space-y-3 text-xs">
              <div>
                <Label className="text-xs font-semibold">Токен или ссылка</Label>
                <Input
      ...
      {isTokenModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-level-1 border border-accent/40 rounded-xl p-5 w-full max-w-lg space-y-4 max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="space-y-3 text-xs">
              <div>
                <Label className="text-xs font-semibold">Название / Описание</Label>
                <Input
                  value={tokenName}
              ...
              <div className="flex items-center justify-between p-2.5 rounded-lg bg-level-2 border border-accent/20">
                <span className="font-medium">Управление плеером и виджетом</span>
                <Switch
              ...
              <div className="flex items-center justify-between p-2.5 rounded-lg bg-level-2 border border-accent/20">
                <span className="font-medium">Доступ ко всем плейлистам</span>
                <Switch
                ...
                <div className="space-y-2 pt-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-text-secondary">
                      Выбрано: {Object.keys(selectedPlaylists).length}
                    </span>
                  </div>
      ...
      {isDirectAddOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-level-1 border border-accent/40 rounded-xl p-5 w-full max-w-lg space-y-4 max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="space-y-3 text-xs">
              <div>
                <Label className="text-xs font-semibold">User ID (UUID)</Label>
                <Input
                  value={targetUserId}
              ...
              <div>
                <Label className="text-xs font-semibold">Имя / Заметка</Label>
                <Input
                  value={tokenName}
                  onChange={(e) => setTokenName(e.target.value)}
