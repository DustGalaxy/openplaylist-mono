...
interface SubscriptionCreateModalProps {
  ...
  targetName: string
  onCreate: (settings: SubscriptionSettings) => Promise<void>
  onClose: () => void
}
...
> = ({ isOpen, targetType, targetName, onCreate, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true)
    try {
      await onCreate({ allowed_event_types: allowedEvents })
      onClose()
    } catch (error) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="space-y-1">
              <DialogTitle className="text-lg font-semibold text-text-main leading-none">
                {t('notifications.create.title', 'Оформить подписку')}
              </DialogTitle>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <DialogFooter className="p-6 bg-level-2 border-t border-level-3 sm:justify-end gap-2 sm:gap-2">
            ...
            <Btn
              disabled={isSaving}
              type="submit"
              className="px-4 py-2 text-sm font-medium rounded-lg flex items-center gap-2 bg-level-2  text-text-main border-none"
            >
              {isSaving ? (
              ...
              {t('notifications.create.submit_btn', 'Подписаться')}
            </Btn>
          </DialogFooter>
        </form>
      </DialogContent>
