...
import { OrderModeToggle, SortButtons } from '@/features/playlist/components/sortPanel'
...
import { filterTabActiveClass, filterTabBaseClass, filterTabInactiveClass } from '@/features/landing/styles'
...
export default function ViewerQueuePanel({
  ...
  const updateSort = (patch: Partial<SortSettings>) => onSortChange({ ...sort, ...patch })
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap gap-2 items-center">
        <button
          ...
          className={cn(
            ...
            sort.order_mode === "host" ? filterTabActiveClass : filterTabInactiveClass,
          )}
        >
        ...
        <OrderModeToggle value={sort.order_mode} onChange={(order_mode) => updateSort({ order_mode })} />

        {sort.order_mode !== "host" && <><div className="w-px h-6 bg-text-main/40" /><SortButtons sortSettings={sort} onChange={updateSort} /></>}
      </div>
      ...
      <ReorderableList
        ...
        mode="dnd"
        onReorder={(ids) => onSortChange({ ...sort, order_mode: 'free', manual_order_ids: ids })}
        onStep={() => { }}
        ...
        renderGhost={(track) => (
          <MiniCardDragGhost title={track.title} duration={formatTime(track.duration ? +track.duration : 0)} />
        )}
      />
    </div>
  )
