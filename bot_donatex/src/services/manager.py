...
class SignalRManager(IManager):
    def __init__(self) -> None:
        super().__init__()
        self.connections: list[SignalRListener] = []

    async def add_connection(self, data: ConnectionData):
        logger.info(f"Adding DonateX connection for platform_user_id='{data.platform_user_id}' (user_id={data.user_id})...")
    ...
    async def start(self):
        ...
        logger.info("Fetching initial users list from backend via RPC...")
        ...
        logger.info(f"Completed initial connection setup: {successful_connections}/{len(users)} active.")

    async def stop(self):
        """Корректная остановка всех соединений."""
        logger.info(f"Stopping all active DonateX connections ({len(self.connections)} total)...")
        ...
        logger.info("All DonateX connections stopped.")
    ...
    async def run_connection(self, client: SignalRListener):  # pyright: ignore[reportIncompatibleMethodOverride]
        try:
            ...
            logger.error(f"Error while running connection for platform_user_id='{client.platform_user_id}': {e}", exc_info=True)
    ...
    async def stop_connection(self, client: SignalRListener):  # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        try:
            ...
            logger.error(f"Error during client.stop() for platform_user_id='{client.platform_user_id}': {e}", exc_info=True)
        finally:
