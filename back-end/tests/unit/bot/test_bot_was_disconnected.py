...
async def test_bot_was_disconnected_integration_not_found(auth_service, mock_db_session):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await auth_service.bot_was_disconnected(
            mock_db_session, IntegrationPlatform.TWITCH, "twitch_123"
        )
...
async def test_bot_was_disconnected_success(auth_service, mock_db_session):
    ...
    result = await auth_service.bot_was_disconnected(
        mock_db_session, IntegrationPlatform.TWITCH, platform_user_id
    )
