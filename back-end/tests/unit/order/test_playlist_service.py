...
@pytest.mark.asyncio
async def test_web_extra_data_fetches_single_playlist_by_id(
    mock_session, mock_settings_service, mocker
):
    ...
    mocker.patch("src.services.playlist_service.get_settings_service", return_value=mock_settings_service)
    ...
    assert len(tracks) == 1
    assert tracks[0] == (order_domain, playlist.id)
    assert errors == []


@pytest.mark.asyncio
async def test_non_web_extra_data_fetches_by_source(
    mock_session, mock_settings_service, mocker
):
    ...
    mock_repo.get_user_playlists_by_sourse.assert_awaited_once_with(
        mock_session, user.id, event.owner_platform_id, event.source
    )
...
@pytest.mark.asyncio
async def test_external_requests_blocked_when_not_from_owner(
    mock_session, mock_settings_service, mocker
):
    ...
    assert tracks == []
    assert len(errors) == 1
    err_msgs, err_name, err_id = errors[0]
    assert err_msgs == ["Playlist is not active"]
    assert err_name == "Locked Playlist"
    assert err_id == playlist.id
...
@pytest.mark.asyncio
async def test_owner_bypasses_external_request_restriction(
    mock_session, mock_settings_service, mocker
):
...
@pytest.mark.asyncio
async def test_validation_failure_adds_error(
    mock_session, mocker
):
    ...
    assert err_name == playlist.name
    assert err_id == playlist.id
    mock_repo.add_order_to_playlist.assert_not_called()


@pytest.mark.asyncio
async def test_validation_passes_track_added(
    mock_session, mock_settings_service, mocker
):
    ...
    assert errors == []
    assert tracks == [(order_domain, playlist.id)]


@pytest.mark.asyncio
async def test_mixed_playlists_one_blocked_one_valid(
    mock_session, mock_settings_service, mocker
):
    ...
    assert len(tracks) == 1
    assert tracks[0] == (order_domain, open_pl.id)
...
@pytest.mark.asyncio
async def test_validate_track_returns_none_treated_as_no_errors(
    mock_session, mocker
):
...
@pytest.mark.asyncio
async def test_validate_track_called_with_correct_args(
    mock_session, mock_settings_service, mocker
):
    ...
    mock_settings_service.validate_track.assert_awaited_once_with(mock_session, playlist, event, user)


@pytest.mark.asyncio
async def test_multiple_valid_playlists_all_tracks_added(
    mock_session, mock_settings_service, mocker
):
    ...
    assert errors == []
    assert len(tracks) == 3
    for i, (od, pl_id) in enumerate(tracks):
        assert od == order_domains[i]
        assert pl_id == playlists[i].id