...
def auth_service():
    ...
    return AuthService(user_repo, link_repo, token_vault_repo)
...
async def test_login_by_social_existing_link(auth_service, mock_db_session, mocker):
    ...
    existing_link = LinkedAccountsDomain(
        id=link_id, user_id=user_id, platform=IntegrationPlatform.TWITCH,
        platform_user_id="twitch_123", platform_username="twitch_user",
        platform_avatar_url="url", platform_user_email="user@test.com",
        bot_connection=False, bot_settings=None, is_dead=False,
    ...
    existing_user = AuthUserSchema(
        id=user_id, username="main_user", email="user@test.com", email_confirmed=True,
        is_active=True, last_login=datetime.now(), created_at=datetime.now(), updated_at=datetime.now()
    )
...
async def test_login_by_social_email_collision(auth_service, mock_db_session, mocker):
    ...
    platform_result = PlatformAuthResult(
        user=PlatformUser(id="twitch_777", username="new_twitch_user", avatar_url="avatar_url", email="collision@test.com", email_verified=True),
    ...
    clashing_link = LinkedAccountsDomain(
        id=uuid4(), user_id=uuid4(), platform=IntegrationPlatform.TWITCH,
        platform_user_id="twitch_old", platform_username="old_user",
        platform_avatar_url="url", platform_user_email="collision@test.com",
        bot_connection=False, bot_settings=None, is_dead=False,
        created_at=datetime.now(), updated_at=datetime.now()
    )
...
async def test_login_by_social_email_confirmation(auth_service, mock_db_session, mocker):
    ...
    user_id = uuid4()
    clashing_link = LinkedAccountsDomain(
        id=uuid4(), user_id=user_id, platform=IntegrationPlatform.TWITCH,
        platform_user_id="twitch_old", platform_username="old_user",
        platform_avatar_url="url", platform_user_email="collision@test.com",
        bot_connection=False, bot_settings=None, is_dead=False,
        created_at=datetime.now(), updated_at=datetime.now()
...
async def test_login_by_social_user_email_exists_level_3(auth_service, mock_db_session, mocker):
    ...
    platform_result = PlatformAuthResult(
        user=PlatformUser(id="twitch_888", username="twitch_user", avatar_url="url", email="collision@test.com", email_verified=True),
    ...
    existing_user = AuthUserSchema(
        id=user_id, username="existing_user", email="collision@test.com", email_confirmed=True,
        is_active=True, last_login=datetime.now(), created_at=datetime.now(), updated_at=datetime.now()
    )
    auth_service.user_repo.get_one = AsyncMock(return_value=existing_user)
    ...
    created_link = LinkedAccountsDomain(
        id=link_id, user_id=user_id, platform=IntegrationPlatform.TWITCH,
        platform_user_id="twitch_888", platform_username="twitch_user",
        platform_avatar_url="url", platform_user_email="collision@test.com",
        bot_connection=False, bot_settings=None, is_dead=False,
        created_at=datetime.now(), updated_at=datetime.now()
    )
...
async def test_login_by_social_new_user_level_4(auth_service, mock_db_session, mocker):
    ...
    created_user = AuthUserSchema(
        id=new_user_id, username="new_user", email="new@test.com", email_confirmed=True,
        is_active=True, last_login=datetime.now(), created_at=datetime.now(), updated_at=datetime.now()
    )
    created_link = LinkedAccountsDomain(
        id=new_link_id, user_id=new_user_id, platform=IntegrationPlatform.TWITCH,
        platform_user_id="twitch_777", platform_username="new_user",
        platform_avatar_url="url", platform_user_email="new@test.com",
        bot_connection=False, bot_settings=None, is_dead=False,
        created_at=datetime.now(), updated_at=datetime.now()
    )
