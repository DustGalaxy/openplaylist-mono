...
class OrderService:
    ...
    def get_from_cache(self, video_id: str) -> dict | None:
        raw_data = get_broker().get(video_id)
        if not raw_data or str(raw_data) == "None":
            return None
    ...
    async def init_order(
        ...
        return OrderCreate(
            ...
            requester_id=order.requester_id if order.requester_id else None,
