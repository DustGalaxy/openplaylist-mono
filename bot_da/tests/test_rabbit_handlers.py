...
from adapters._rabbit.handlers import add_connection, disconnect_from_da
...
async def test_add_connection_handler_success():
    ...
    conn_data = ConnectionData(
        user_id=uuid4(),
        platform_user_id="da_user_test_1",
        access_token="tok_1",
        refresh_token="ref_1",
        expires_at=1000,
    )
...
async def test_add_connection_handler_without_manager():
    ...
    conn_data = ConnectionData(
        user_id=uuid4(),
        platform_user_id="da_user_test_2",
        access_token="tok_2",
        refresh_token="ref_2",
        expires_at=1000,
    )
...
async def test_disconnect_handler_success():
    ...
    mock_listener.platform_user_id = "da_user_disc_1"
    ...
    result = await disconnect_from_da("da_user_disc_1")
    assert result is True
...
async def test_disconnect_handler_not_found_returns_true():
    ...
    result = await disconnect_from_da("da_user_non_existent")
    assert result is True
