...
from dto.playlist import NewPlaylist, PlayNow, PlaylistBaseinfo, ReadPlaylist, ReadPlaylistPreview
...
async def patch_playlist_settings(
    ...
    try:
        ...
        await broker.publish(
            ReadPlaylistSettings.model_validate(plst_settings), playlist_settings_changed, main_exchange
        )

        # redis schema - {user_id}:{playlist_name}:settings
        redis_adapter.set(f"{current_user.id}:{playlist_name}:settings", plst_settings.model_dump_json())
...
async def set_play_now_for_playlist(
    ...
    try:
        ...
        await broker.publish(playnow, playlist_track_playnow, main_exchange)
    except NotFoundException:
        raise HTTPException(status_code=404, detail="Playlist not found")
...
async def delete_track_from_playlist(
    ...
    try:
        ...
        await broker.publish(
            {"track_id": track_id, "playlist_id": str(playlist_id)}, playlist_track_deleted, main_exchange
        )
    except NotFoundException:
        raise HTTPException(status_code=404, detail="Playlist not found")
