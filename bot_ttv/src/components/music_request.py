...
from src.utils import extract_data_from_msg, extract_youtube_video_id, get_user_id, is_broadcaster_or_moderator
...
class MusicRequest(commands.Component):
    ...
    async def mr(self, ctx: commands.Context, playlist_name: str, yt_url: str) -> None:
        music_request_enable = bool(redis_adapter.get(f"{ctx.channel.name}:mr:enable"))

        LOGGER.info(f"yt_url: {yt_url}, playlist_name: {playlist_name}")
    ...
    async def music_request_by_points(self, ctx: commands.Context, msg: str) -> None:
        ...
        yt_url = data["yt_url"]
        playlist_name = data["playlsit_name"]
